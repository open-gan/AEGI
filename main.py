...
def parse_arg():
    ...
    parser.add_argument('--noise_dim', type=int, default=256, help='latent space')
    ...
    parser.add_argument('--save_step', type=int, default=1, help='epoch for saving mdoel')

    parser.add_argument('--channels', default="32, 64, 128, 256, 512", type=str, help='channels for each layer')
    parser.add_argument('--trainsize', type=int, default=29000, help='training sample')
    parser.add_argument('--input_height', type=int, default=128, help='input image size')
    parser.add_argument('--input_width', type=int, default=None, help='input image size')
    parser.add_argument('--output_height', type=int, default=128, help='output image size')
    ...
    parser.add_argument('--m_plus', type=float, default=140.0, help='m in formula')
    parser.add_argument('--weight_neg', type=float, default=0.5, help='alpha * loss_adv， alpha in formula')
    parser.add_argument('--weight_rec', type=float, default=0.2, help='lambda * ae_loss， lambda in formula')
...
def main():
    ...
    def train_vae(epoch, iteration, real_images, cur_iter, statistic):
        ...
        if cur_iter % config.sample_step == 0:
            ...
            # mu, logvar = model.encode(real_images)
            # rec_images = model.generator(mu)
    ...
    for epoch in range(config.start_epoch, config.nEpochs + 1):
        ...
        if epoch % config.save_step == config.save_step - 1:
            save_checkpoint(model, ema, epoch, 0, statistic, '')
